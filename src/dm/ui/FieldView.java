		...
		for(int i=0;i<5;i++){
			try{
				...
				if(card.getState()==CardState.FACE_UP_ATTACK)
						bufferedImage = im.mixImages(bufferedImage,file,new Dimension(width,height),
								card_dim, monster_x+70*i +i+1,  monster1_y);
				else
				if(card.getState()==CardState.FACE_UP_DEFENSE_POS)
					bufferedImage = im.mixImages(bufferedImage,it.rotateImage(ImageIO.read(file), -90, AffineTransformOp.TYPE_BICUBIC),new Dimension(width,height),
							card_dim_deffense, monster_x+70*i +i+1 - defense_bias,  monster1_y-defense_bias);
				else if(card.getState()==CardState.FACE_DOWN)
						bufferedImage = im.mixImages(bufferedImage,it.rotateImage(ImageIO.read(face_down_file), -90, AffineTransformOp.TYPE_BICUBIC),new Dimension(width,height),
								card_dim_deffense, monster_x+70*i +i+1 - defense_bias,  monster1_y-defense_bias);
...
//				e2.printStackTrace();
		...
		for(int i=0;i<5;i++){
				...
				try{
					...
					System.out.println("STATE  " +card.getState() );
					if(card.getState()==CardState.FACE_UP_ATTACK)
							bufferedImage = im.mixImages(bufferedImage,it.rotateImage(ImageIO.read(file), 180, AffineTransformOp.TYPE_BICUBIC),new Dimension(width,height),
									card_dim, monster_x+70*i +i+1,  monster2_y);
					else
					if(card.getState()==CardState.FACE_UP_DEFENSE_POS)
						bufferedImage = im.mixImages(bufferedImage,it.rotateImage(ImageIO.read(d_file), 90, AffineTransformOp.TYPE_BICUBIC),new Dimension(width,height),
								card_dim_deffense, monster_x+70*i +i+1 - defense_bias,  monster2_y - defense_bias);
					else if(card.getState()==CardState.FACE_DOWN)
							bufferedImage = im.mixImages(bufferedImage,it.rotateImage(ImageIO.read(face_down_file), 90, AffineTransformOp.TYPE_BICUBIC),new Dimension(width,height),
									card_dim_deffense,monster_x+70*i +i+1 - defense_bias,  monster2_y - defense_bias);
				}catch(NullPointerException e){
...
//					e2.printStackTrace();
		...
		//Spell 1
		for(int i=0;i<5;i++){
			bufferedImage = im.mixImages(bufferedImage,d_file,new Dimension(width,height),
		...
		for(int i=0;i<5;i++){
			bufferedImage = im.mixImages(bufferedImage,d_file,new Dimension(width,height),
					card_dim, monster_x+70*i +i+1,  spell2_y);
		}
		
		bufferedImage = im.mixImages(bufferedImage,d_file,new Dimension(width,height),
				card_dim, extra_left_x,  deck1_y);
		bufferedImage = im.mixImages(bufferedImage,d_file,new Dimension(width,height),
				card_dim, extra_left_x,  graveyard1_y);
		bufferedImage = im.mixImages(bufferedImage,d_file,new Dimension(width,height),
				card_dim, extra_right_x,  deck1_y);
		bufferedImage = im.mixImages(bufferedImage,d_file,new Dimension(width,height),
				card_dim, extra_right_x,  graveyard1_y);
		
		bufferedImage = im.mixImages(bufferedImage,d_file,new Dimension(width,height),
				card_dim, extra_left_x,  deck2_y);
		bufferedImage = im.mixImages(bufferedImage,d_file,new Dimension(width,height),
				card_dim, extra_left_x,  graveyard2_y);
		bufferedImage = im.mixImages(bufferedImage,d_file,new Dimension(width,height),
				card_dim, extra_right_x,  deck2_y);
		bufferedImage = im.mixImages(bufferedImage,d_file,new Dimension(width,height),
				card_dim, extra_right_x,  graveyard2_y);
